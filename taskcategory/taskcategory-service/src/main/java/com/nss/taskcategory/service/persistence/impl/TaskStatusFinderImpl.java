...
@Component(service = TaskStatusFinder.class)
public class TaskStatusFinderImpl extends TaskStatusFinderBaseImpl implements TaskStatusFinder {
	@Override
	...
	public static final String COUNT_BY_BASE_SEARCH = TaskStatusFinder.class.getName() + ".countByBaseSearch";
	public static final String FIND_BY_BASE_SEARCH = TaskStatusFinder.class.getName() + ".findByBaseSearch";
	...
	private List<TaskStatus> findByBaseSearch(long companyId, long groupId, int active, String[] names, 
			String[] codes, String[] descriptions, boolean andOperator,
		...
		try {
			...
			SQLQuery q = session.createSQLQuery(sql);
			q.addEntity("TaskStatus", TaskStatusImpl.class);
			QueryPos qPos = QueryPos.getInstance(q);
			qPos.add(companyId);
			if (groupId > 0) {
				qPos.add(groupId);
			...
			qPos.add(codes, 2);
			qPos.add(descriptions, 2);
			return (List<TaskStatus>) QueryUtil.list(q, getDialect(), start, end);
		} catch (Exception e) {
			throw new SystemException(e);
		} finally {
			closeSession(session);
