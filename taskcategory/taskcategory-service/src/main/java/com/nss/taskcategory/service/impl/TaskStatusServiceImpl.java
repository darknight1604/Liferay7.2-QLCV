...
import com.nss.taskcategory.service.base.TaskStatusServiceBaseImpl;
 ...
 *
 * <p>
 * All custom service methods should be put in this class. Whenever methods are added, rerun ServiceBuilder to copy their definitions into the <code>com.nss.taskcategory.service.TaskStatusService</code> interface.
 *
 * <p>
...
public class TaskStatusServiceImpl extends TaskStatusServiceBaseImpl {
	 ...
	 * NOTE FOR DEVELOPERS:
	 *
	 * Never reference this class directly. Always use <code>com.nss.taskcategory.service.TaskStatusServiceUtil</code> to access the task status remote service.
	 */
	private static volatile PortletResourcePermission _portletResourcePermission = PortletResourcePermissionFactory.getInstance(TaskStatusServiceImpl.class, "_portletResourcePermission", TaskCategoryResourceConstants.RESOURCE_NAME);
	...
	public TaskStatus addTaskStatus(long userId, String name, 
		...
		_portletResourcePermission.check(getPermissionChecker(), serviceContext.getScopeGroupId(), TaskCategoryActionKeys.ADD_TASKSTATUS);
		
		return taskStatusLocalService.addTaskStatus(userId, name, 
				code, priority, active, description, serviceContext);
	}
	
	public TaskStatus updateTaskStatus(long taskTypeId, String name, 
			String code, int priority, boolean active, String description, ServiceContext serviceContext) throws PortalException {
		_portletResourcePermission.check(getPermissionChecker(), serviceContext.getScopeGroupId(), TaskCategoryActionKeys.UPDATE);
		
		return taskStatusLocalService.updateTaskStatus(taskTypeId, name, 
				code, priority, active, description, serviceContext);
	}
	
	public boolean hasAddPermission(long groupId) throws PrincipalException {
		return _portletResourcePermission.contains(getPermissionChecker(), groupId, TaskCategoryActionKeys.ADD_TASKSTATUS);
	}
