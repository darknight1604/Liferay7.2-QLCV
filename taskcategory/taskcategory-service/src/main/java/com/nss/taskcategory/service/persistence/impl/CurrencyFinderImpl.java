...
@Component(service = CurrencyFinder.class)
public class CurrencyFinderImpl extends CurrencyFinderBaseImpl implements CurrencyFinder {
	@Override
	...
	public static final String COUNT_BY_BASE_SEARCH = CurrencyFinder.class.getName() + ".countByBaseSearch";
	public static final String FIND_BY_BASE_SEARCH = CurrencyFinder.class.getName() + ".findByBaseSearch";
	...
	private List<Currency> findByBaseSearch(long companyId, long groupId, int active, String[] names, 
			String[] codes, String[] descriptions, boolean andOperator,
		...
		try {
			...
			SQLQuery q = session.createSQLQuery(sql);
			q.addEntity("Currency", CurrencyImpl.class);
			QueryPos qPos = QueryPos.getInstance(q);
			qPos.add(companyId);
			if (groupId > 0) {
				qPos.add(groupId);
			...
			qPos.add(codes, 2);
			qPos.add(descriptions, 2);
			return (List<Currency>) QueryUtil.list(q, getDialect(), start, end);
		} catch (Exception e) {
			throw new SystemException(e);
		} finally {
			closeSession(session);
