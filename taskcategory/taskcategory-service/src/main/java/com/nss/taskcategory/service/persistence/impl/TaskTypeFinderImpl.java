...
@Component(service = TaskTypeFinder.class)
public class TaskTypeFinderImpl extends TaskTypeFinderBaseImpl implements TaskTypeFinder {
	@Override
	...
	public static final String COUNT_BY_BASE_SEARCH = TaskTypeFinder.class.getName() + ".countByBaseSearch";
	public static final String FIND_BY_BASE_SEARCH = TaskTypeFinder.class.getName() + ".findByBaseSearch";
	...
	private List<TaskType> findByBaseSearch(long companyId, long groupId, int active, String[] names, 
			String[] codes, String[] descriptions, boolean andOperator,
		...
		try {
			...
			SQLQuery q = session.createSQLQuery(sql);
			q.addEntity("TaskType", TaskTypeImpl.class);
			QueryPos qPos = QueryPos.getInstance(q);
			qPos.add(companyId);
			if (groupId > 0) {
				qPos.add(groupId);
